...
/**
 * Note for syscall registers usage (aarch64):
 *   - x8 is the syscall number.
 *   - x0 is the return value.
 *   - x0 is also the 1st argument.
 *   - x1 is the 2nd argument.
 *   - x2 is the 3rd argument.
 *   - x3 is the 4th argument.
 *   - x4 is the 5th argument.
 *   - x5 is the 6th argument.
 */

#define __do_syscall0(num) ({						\
	register long _num __asm__("x8") = (num);			\
	register long _arg1 __asm__("x0") = (num);			\
									\
	__asm__ volatile (						\
		"svc #0"						\
		: "=r"(_arg1)						\
